...
upnpcversions=`curl http://miniupnp.free.fr/files/rest.php/tags | jq -r ".tags.miniupnpc | .[] | .file"`

## Créer le conteneur docker qui servira à compiler le package.

for upnpcversion in $upnpcversions
do 
    build=`echo $upnpcversion | cut -d '-' -f 2 |cut -d '.' -f 1-3`

    cat <<ENDOFFILE >Dockerfile
    FROM alpine:latest
    RUN apk add alpine-sdk
    RUN wget http://miniupnp.free.fr/files/download.php?file=$upnpcversion -O latest.tar.gz ; tar -zxf latest.tar.gz ;cd miniupnpc-* ; make; mkdir /deploy ; make DESTDIR=/deploy install; make install
...
ENDOFFILE
    ...
    sudo docker save --output miniupnpc.$build.tar miniupnpc
    sudo chown $USER:$USER miniupnpc.$build.tar
    gzip miniupnpc.$build.tar
done
